...
/// <summary>
/// Выполняющийся прогон пробы: то, что видно в панели операций, и источник сырых измерений для графика.
/// </summary>
/// <remarks>
/// Очередь сырых измерений заполняется фоновым потоком и разбирается вызовом
...
public sealed class ActiveRunViewModel : ActiveOperationViewModel
{
    ...
    /// <summary>
    /// Забирает накопленные сырые измерения. Вызывается только из потока интерфейса.
    /// </summary>
