...
public sealed record Device
{
    ...
    /// </summary>
    /// <remarks>
    /// Различать догадку и факт показ обязан по-прежнему, но не знаком препинания:
    /// «сервер?» читалось как опечатка или как вопрос продукта к оператору, а не как
    /// оговорка. Оговорка осталась словами и тоном — приглушённый тег и подсказка
    /// «категория определена по косвенным признакам»; там, где есть место на фразу,
    /// продукт говорит её целиком. Догадка, выданная за факт, — то же враньё, что
    /// связь на карте без оговорки, и признак нужен именно для этого.
    /// </remarks>
    public bool RoleIsGuessed { get; init; }

    /// <summary>Роль для показа. Догадка помечается видом тега, а не вопросом.</summary>
